...
describe('Reauthentication modal submit', () => {
  ...
  it('shares one active prompt for concurrent callers', async () => {
    mockPost.mockResolvedValue({ data: { status: 'success' } });

    let p1;
    let p2;
    await act(async () => {
      p1 = promptReauthentication({ action: 'users.destroy' });
      p2 = promptReauthentication({ action: 'users.destroy' });
      await new Promise(r => setTimeout(r, 0));
    });
  ...
  it('cancel clears password and rejects without POSTing', async () => {
    let rejected = false;
    await act(async () => {
      promptReauthentication({ action: 'users.destroy' }).catch(() => {
        rejected = true;
      });
      await new Promise(r => setTimeout(r, 0));
    });
