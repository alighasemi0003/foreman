...
export const submitLegacyWithReauth = async (
  ...
  if (response.ok) {
    ...
    // Never treat JSON/error bodies as file downloads (e.g. key-pair PEM).
...
const onReauthGetLinkClick = async event => {
  const link = event.target.closest?.(
    'a[data-foreman-reauth="true"], a[data-foreman-reauth="1"]'
  );
  if (!link || !document.contains(link)) return;
  // data-method links are handled by onMethodLinkClick
...
export const installLegacyReauthentication = () => {
  ...
  document.addEventListener('click', onReauthGetLinkClick, true);
