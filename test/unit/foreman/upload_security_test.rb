...
class Foreman::UploadSecurityTest < ActiveSupport::TestCase
  ...
  test 'valid_jpeg_avatar? rejects HTML SVG PNG and executables' do
  ...
  test 'validate_cacert! accepts single certificate' do
    pem = File.read(Rails.root.join('test/static_fixtures/certificates/example.com.crt'))
    assert Foreman::UploadSecurity.validate_cacert!(pem)
  end

  test 'validate_cacert! accepts certificate bundle' do
    a = File.read(Rails.root.join('test/static_fixtures/certificates/example.com.crt'))
    b = File.read(Rails.root.join('test/static_fixtures/certificates/example2.com.crt'))
    assert Foreman::UploadSecurity.validate_cacert!(a + "\n" + b)
  end

  test 'validate_cacert! rejects garbage and private keys' do
    assert_raises(Foreman::UploadSecurity::InvalidCacert) do
      Foreman::UploadSecurity.validate_cacert!('not a certificate at all')
    end
    assert_raises(Foreman::UploadSecurity::InvalidCacert) do
      Foreman::UploadSecurity.validate_cacert!("-----BEGIN CERTIFICATE-----\nYQ==\n-----END CERTIFICATE-----\n")
    end
    assert_raises(Foreman::UploadSecurity::InvalidCacert) do
      Foreman::UploadSecurity.validate_cacert!("-----BEGIN PRIVATE KEY-----\nYQ==\n-----END PRIVATE KEY-----\n")
    end
  end

  test 'validate_cacert! rejects oversized PEM' do
    pem = File.read(Rails.root.join('test/static_fixtures/certificates/example.com.crt'))
    huge = pem + ('A' * (Foreman::UploadSecurity::MAX_CACERT_BYTES + 1))
    assert_raises(Foreman::UploadSecurity::InvalidCacert) do
      Foreman::UploadSecurity.validate_cacert!(huge)
    end
  end
  ...
  test 'safe_download_filename strips path and control characters' do
    ...
    assert_equal 'a_b.erb', Foreman::UploadSecurity.safe_download_filename('a"b.erb')  end
