...
class Api::V2::UploadPayloadSizeTest < ActionController::TestCase
  ...
  test 'facts proceeds when body is within limit' do
    ...
    Host::Managed.any_instance.stubs(:import_host).returns(FactoryBot.build_stubbed(:host))
    # detect_host_type.import_host path:
    Api::V2::HostsController.any_instance.stubs(:detect_host_type).returns(Host::Managed)
