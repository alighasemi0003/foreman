...
class Api::V2::SettingsControllerReauthenticationTest < ActionController::TestCase
  ...
  setup do
    ...
    # Session-backed UI call into API — not machine/token auth
    @request.session[:api_authenticated_session] = false
    # Intentionally stale — meta policy must still require reauth
    users(:admin).update_column(:has_active_session, true)
