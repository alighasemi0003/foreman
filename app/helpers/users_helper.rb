module UsersHelper
  ...
  def user_action_buttons(user, additional_actions = [])
    ...
    if user != User.current && user.has_attribute?(:has_active_session) && user.has_active_session?
      additional_actions << display_link_if_authorized(_("Terminate Session"),
        hash_for_terminate_active_session_user_path(:id => user.id).merge(:auth_object => user, :permission => "edit_users"),
        :method => :patch, :id => user.id,
        :data => {
          :confirm => _("Terminate active session for %s?") % user.name,
          :foreman_reauth => true,
        })
    end
